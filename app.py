...
# 使用 f-string 時，CSS 的花括號必須雙寫 {{ }} 以免產生解析錯誤
st.markdown(f"""
    <style>
    /* 🌌 背景設定 */
    .stApp {{ 
...
# 📅 顯示日期區間
st.markdown(f'<div class="status-bar">OBSERVATION PERIOD: {date_range_str}</div>', unsafe_allow_html=True)
...
csv_url = f"https://docs.google.com/spreadsheets/d/{{SHEET_ID}}/export?format=csv".format(SHEET_ID=SHEET_ID)
...
try:
            ...
            <div class="ai-monitor-wrapper">
                <div class="ai-monitor-box">
                    ...
                    <div style="color:#ffffff; line-height:1.6;">
                        ...
                        分析摘要：在 <strong>{date_range_str}</strong> 區間內，該議題在各類媒體端展現極高擴大動能。
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    for i, (_, row) in enumerate(ranked_df.head(15).iterrows()):
        st.markdown(f"""
            ...
            """.replace("{{i+1}}", str(i+1)).replace("{{topic}}", str(row.iloc[0])).replace("{{time}}", str(row.iloc[3])), unsafe_allow_html=True)
